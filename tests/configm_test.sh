...
function save_config_file_CHECK_CONFIGS_DIRECTORY_test()
{
  ...
  local current_path=$PWD
  local ret=0
  kw_config_files_path=$current_path/$test_path
  ...
  # Check if all the expected files was created
  assertTrue "The configs dir was not created" '[[ -d $kw_config_files_path/configs ]]'
  assertTrue "The repository configs does not have .git" '[[ -d $kw_config_files_path/configs/.git ]]'
  assertTrue "The metadata dir is not available" '[[ -d $kw_config_files_path/configs/metadata ]]'
  assertTrue "The configs dir is not available" '[[ -d $kw_config_files_path/configs/configs ]]'
...
function save_config_file_CHECK_SAVED_CONFIG_FILE_test()
{
  ...
  assertTrue "Failed to find $NAME_1" '[[ -f $kw_config_files_path/configs/configs/$NAME_1 ]]'
  assertTrue "Failed the metadata related to $NAME_1" '[[ -f $kw_config_files_path/configs/metadata/$NAME_1 ]]'

  cd $test_path
  ret=$(save_config_file $NO_FORCE $NAME_2)
  cd $current_path

  assertTrue "Failed to find $NAME_2" '[[ -f $kw_config_files_path/configs/configs/$NAME_2 ]]'
  assertTrue "Failed the metadata related to $NAME_2" '[[ -f $kw_config_files_path/configs/metadata/$NAME_2 ]]'

  local tmp=$(cat $kw_config_files_path/configs/configs/$NAME_2)
...
function save_config_file_CHECK_DESCRIPTION_test()
{
  ...
  local tmp=$(cat $kw_config_files_path/configs/metadata/$NAME_1)
  ...
  tmp=$(cat $kw_config_files_path/configs/metadata/$NAME_2)
...
function list_configs_OUTPUT_test()
{
  ...
  # There's no configs yet, initialize it
  ret=$(list_configs)
  assertTrue "We 'Name' in the output" '[[ $ret =~ Name ]]'
  assertTrue "We 'Description' in the output" '[[ $ret =~ Description ]]'
  assertTrue "We $NAME_1 in the output" '[[ $ret =~ $NAME_1 ]]'
  assertTrue "We $DESCRIPTION_1 in the output" '[[ $ret =~ $DESCRIPTION_1 ]]'
  assertTrue "We $NAME_2 in the output" '[[ $ret =~ $NAME_2 ]]'
  assertTrue "We $DESCRIPTION_2 in the output" '[[ $ret =~ $DESCRIPTION_2 ]]'
